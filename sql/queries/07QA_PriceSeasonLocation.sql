...
SET @dynamic_query = CONCAT(@dynamic_query, '
	SELECT
		...
		sp.active_ind,
		sp.display_ind ,
		ps.PricingSeasonCode ps_code,
		ps.Description AS PricingSeason ,
		spp.sale_location_code ,
		l.description AS SaleLocation,
		spp.price,
		spp.price_allocation_ind,
		spp.discount_allocation_ind,
		spp.commission_allocation_ind,
		MIN(psd.PricingSeasonDate) AS [Min Date],
		MAX(psd.PricingSeasonDate) AS [Max Date],
		spt.tax_code,
	...
	FROM s_display_category sdc ( NOLOCK )
		...
		JOIN dbo.s_season_location_product_price spp WITH (NOLOCK)
		ON sph.product_header_code = spp.product_header_code


		JOIN s_product sp ( NOLOCK )
		ON spp.product_code = sp.product_code
		--AND sp.active_ind = ''Y''

		--	-- Pricing Season by Location
		--JOIN	s_season_location_product_price sslpp ( NOLOCK )
		--	ON sphl.product_header_code = sslpp.product_header_code
		--	AND sphl.product_code = sslpp.product_code
		--	--and spp.SalesChannelCode = sc.SalesChannelCode

		JOIN PricingSeason ps ( NOLOCK )
		ON spp.PricingSeasonCode = ps.PricingSeasonCode

		JOIN PricingSeasonDate psd ( NOLOCK )
		ON ps.PricingSeasonCode = psd.PricingSeasonCode

		JOIN dbo.s_location l with (NOLOCK)
		on spp.sale_location_code = l.location_code
