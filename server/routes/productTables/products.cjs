...
/** GET /api/products — paged product list with filters */
router.get('/products', async (req, res, next) => {
  try {
    ...
    // Nullify code when expanding category so SQL doesn't restrict to one PHC
    ...
    p.input('code', sql.VarChar(50), effectiveCode ?? null);
    ...
    // Bind @cat
    const catAsInt =
      catValue != null && !Number.isNaN(Number(catValue))
        ? parseInt(catValue, 10)
        : null;
    if (catAsInt != null) {
...
`;
    ...
    res.json({
      ...
      anchorCategory: catAsInt != null ? catAsInt : catValue ?? null,
...
/** GET /api/products/:phc/components — components assigned to a PHC */
