...
// ── Reusable browser ─────────────────────────────────────────────────────────
// Import this anywhere you need to embed the PC browser in a BrowserModal.
        ...
        <ModalSessionProvider>
          <div className="pm-tab-host">
            <div className="pm-tabs-row">
              ...
              <ModalTabButton active={activeTab === 'profile'} onClick={() => setActiveTab('profile')}>Profile</ModalTabButton>
            </div>
            <div className="pm-tab-body pm-form-shell">
              {activeTab === 'general' && <PC_GeneralTab productCode={detail.productCode} isActive form={form} update={update} />}
              {activeTab === 'additional' && <PC_AdditionalTab productCode={detail.productCode} isActive={activeTab === 'additional'} form={form} update={update} />}
              {activeTab === 'profile' && <PC_ProfileTab productCode={detail.productCode} isActive={activeTab === 'profile'} form={form} update={update} />}
            </div>
          </div>
        </ModalSessionProvider>
      </Modal>
...
export default function ManageProductComponent() {
  ...
  return <ProductComponentBrowser initialFocusCode={componentAnchorCode} />;
}